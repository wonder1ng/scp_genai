...
eval_ds = ds["train"].filter(lambda x: bool(x["document"])).shuffle(seed=42).select(range(500))

print(f"학습 데이터 수: {len(train_ds)}, 평가 데이터 수: {len(eval_ds)}")
print(f"예시: {train_ds[0]['document']}, {train_ds[0]['document']}")
...
args = TrainingArguments(
    output_dir=full_path("nsmc_kr_results"),
    eval_strategy="epoch",
    save_strategy="epoch",
...
save_path = full_path("my_local_nsmc_kr_model")
